...
def lesion_toads(T1_image, FLAIR_image):
    ...
    lesion_toads.inputs.outHard = os.path.join(toads_directory, image_name+'_alllabels.nii.gz')
    lesion_toads.inputs.outLesion = os.path.join(toads_directory, image_name+'lesionseg.nii.gz')
    lesion_toads.run()

    # correct labels
    label_transf = {0:0, 1:0, 5:1, 6:1, 14:2, 15:2, 16:2, 17:2, 18:2, 22:2, 24:3, 25:3, 10:3}

    # apply trasnformation to image

    im = nib.load(os.path.join(toads_directory, image_name+'_alllabels.nii.gz'))
    ...
    out_mask = filename.split('.')[0] + '_seg.nii.gz'
    im.get_data()[:] = toads_segmentation
    nib.save(im, os.path.join(toads_directory, out_mask))
